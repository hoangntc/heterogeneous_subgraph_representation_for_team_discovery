...
def read_subgraphs(sub_f, split = True):
    ...
    #     print(f'Number of train {len(train_sub_G)}')
    #     print(f'Number of val {len(test_sub_G)}')
    #     print(f'Number of test {len(val_sub_G)}')
    #     return train_sub_G, train_sub_G_label, test_sub_G, test_sub_G_label, val_sub_G, val_sub_G_label
    
    print(f'Number of train {len(train_sub_G)}')
    print(f'Number of val {len(val_sub_G)}')
    print(f'Number of test {len(test_sub_G)}')
...
def calc_top_k_accuracy(logits, y_true, k=2, normalize=True, labels=None, multilabel_binarizer=None):
    '''
    Calculates the top-k accuracy (only in multilabel_binarizer=None)
    '''
...
def get_neighbor_nodes(stellar_graph, node, meta_paths, radius, n_run=5):
    random_walk = UniformRandomMetaPathWalk(stellar_graph)
    walks = random_walk.run([node], n=n_run, length=radius, metapaths=meta_paths, seed=0)
    return list(set(itertools.chain(*walks)))

def get_component_border_neighborhood_set(stellar_graph, meta_paths, component, k, ego_graph_dict=None):
    '''
    ...
    for node in component_set: 
        if ego_graph_dict == None: # if it hasn't already been computed, calculate the ego graph (i.e. induced subgraph of neighbors centered at node with specified radius)
            ...
            ego_g = get_neighbor_nodes(stellar_graph, node, meta_paths, radius=k)
        else:
