...
def build_model(run_config, trial = None):
    '''
    Creates HSGNN from the hyperparameters specified in the run config
    '''
    ...
    # initialize HSGNN
    print('############################## Initialize model and data')
    # data_keys = [
    #     'neigh_sample_border_size', 'n_anchor_patches_structure', 
    #     'structure_patch_type', 'trainable_cc', 
    #     'n_anchor_patches_N_out', 
    #     'sample_walk_len', 'use_structure', 'compute_similarities', 'n_anchor_patches_pos_out',
    #     'resample_anchor_patches', 'use_neighborhood', 'n_triangular_walks', 'batch_size', 
    #      'random_walk_len', 'use_mpn_projection', 'n_anchor_patches_pos_in', 
    #      'n_processes', 'use_position', 'n_anchor_patches_N_in']

    model = md.HSGNN(
        hyperparameters, 
        run_config["graph_path"], 
        run_config['node_path'],
...
def build_trainer(run_config, hyperparameters, trial = None):
    ...
    trainer_kwargs={'max_epochs': hyperparameters['max_epochs'],
                    "gpus": 0 if 'no_gpu' in run_config else [0],
    ...
    if run_config["optuna"]['pruning']:
        if 'patience' not in run_config['optuna']:
            ...
            trainer_kwargs['early_stop_callback'] = EarlyStopping(
                monitor=run_config['optuna']['monitor_metric'],
                min_delta=0.00,
                patience=run_config['optuna']['patience'],
                verbose=True,
                mode='max'
                )
...
def train_model(run_config, trial = None):
    ...
    for score in model.metric_scores:
        if isinstance(score[run_config['optuna']['monitor_metric']], np.float64):
            np_score = score[run_config['optuna']['monitor_metric']]
        else: np_score = score[run_config['optuna']['monitor_metric']].numpy()
...
def main():
    ...
    run_config['node_path'] = os.path.join(task, "node_types.csv")
    ...
    print('### Run with CPU {}'.format('no_gpu' in run_config))
    # get location of node embeddings
    if embedding_type == 'gin':
        run_config["embedding_path"] = os.path.join(task, "gin_gcn_embeddings.pth")
