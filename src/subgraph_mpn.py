...
class SG_MPN(MessagePassing):
    ...
    def create_patch_embedding_matrix(self,cc_embeds, cc_embed_mask, anchor_embeds, anchor_mask):
        ...
        reshaped_cc_embeds = cc_embeds.view(-1, cc_hidden_dim).to(device=self.device) #(batch_sz * max_n_cc , hidden_dim)
        reshaped_anchor_embeds =  anchor_embeds.view(-1, anchor_hidden_dim).to(device=self.device) #(batch_sz * max_n_cc * n_sampled_patches, hidden_dim)
    ...
    def message(self, x_j, similarity): #default is source to target
        ...
        '''
        return similarity.to(device=self.device) * x_j.to(device=self.device)
